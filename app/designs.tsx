      ...
      {designLinks
        ...
        .map((item, index) => (
          <DesignItem
            ...
            className={designLinks.length > 2 && index === 0 ? 'row-span-2 h-full' : ''}
          />
        ))}
    </section>
  );
...
function DesignItem({ title, href, background, className }: React.ComponentProps<'a'> & { title: string; href: string; background: StaticImageData }) {
  return (
    <Link href={href} className={cn('text-white', className)}>
      <div className={cn(`grid place-items-center bg-cover rounded-lg h-[308px]`, className)} style={{ backgroundImage: `url(${background.src})` }}>
